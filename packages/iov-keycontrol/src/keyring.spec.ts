...
describe("Keyring", () => {
  ...
  it("can serialize and deserialize multiple entries", async () => {
    ...
    const i2a = await entry2.createIdentity(await Ed25519.makeKeypair(await Random.getBytes(32)));
    const i2b = await entry2.createIdentity(await Ed25519.makeKeypair(await Random.getBytes(32)));
    ...
    const i4a = await entry4.createIdentity(await Ed25519.makeKeypair(await Random.getBytes(32)));
    const i4b = await entry4.createIdentity(await Ed25519.makeKeypair(await Random.getBytes(32)));
    const i4c = await entry4.createIdentity(await Ed25519.makeKeypair(await Random.getBytes(32)));
    const i4d = await entry4.createIdentity(await Ed25519.makeKeypair(await Random.getBytes(32)));
